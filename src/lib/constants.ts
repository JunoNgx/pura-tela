export const MIN_COLOUR_COUNT_PALETTE = 2;
export const MAX_COLOUR_COUNT = 7;
export const MIN_DIMENSION = 1;
export const MIN_WIDTH = 1;
export const MAX_WIDTH = 7680;
export const MIN_HEIGHT = 1;
export const MAX_HEIGHT = 4320;
export const HAS_COPY_TIMEOUT_DURATION_MS = 1000;
export const SWATCH_CONFIG_MIN_VALUE = 0;
export const SWATCH_CONFIG_MAX_VALUE = 100;
export const PALETTE_CONFIG_ANGLE_MAX_VALUE = 180;
export const PALETTE_CONFIG_SIZE_MAX_VALUE = 20;
export const PALETTE_CONFIG_POSITION_MAX_VALUE = 100;
export const HORIZON_CONFIG_SIZE_MAX_VALUE = 10;
export const HORIZON_CONFIG_POSITION_MAX_VALUE = 100;
export const POP_ART_SQUARE_CONFIG_SIZE_MIN_VALUE = 0;
export const POP_ART_SQUARE_CONFIG_SIZE_MAX_VALUE = 20;
export const POP_ART_SQUARE_CONFIG_POSITION_MIN_VALUE = 0;
export const POP_ART_SQUARE_CONFIG_POSITION_MAX_VALUE = 100;
export const TWILIGHT_CONFIG_SIZE_MAX_VALUE = 20;
export const TWILIGHT_CONFIG_POSITION_MAX_VALUE = 20;
export const TWILIGHT_CONFIG_RIPPLE_INTENSITY_MAX_VALUE = 20;
export const PIE_MAN_CONFIG_SIZE_MAX_VALUE = 20;
export const PIE_MAN_CONFIG_ANGLE_MAX_VALUE = 360;
export const BAUMKUCHEN_CONFIG_SIZE_MIN_VALUE = 0;
export const BAUMKUCHEN_CONFIG_SIZE_MAX_VALUE = 20;
export const BAUMKUCHEN_CONFIG_POSITION_MIN_VALUE = 0;
export const BAUMKUCHEN_CONFIG_POSITION_MAX_VALUE = 100;

export const DEFAULT_PROMPT_MESSAGE =
    'Enter the theme prompting for the palette (e.g. "autumn"):';
